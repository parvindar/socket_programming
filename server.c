/*
	CS349 
	GroupID = 11
	Parvindar Singh, Kapil Jangid, Rutvik Ghughal
...
void processUserRequests(int clientFD, struct userInfo uInfo) {
	int n;
	char *buff = NULL;
	sendMsgToClient(clientFD, "-> Enter M for Mini-Statement\n-> B for Available Balance exit to terminate");
	while(1) {
		if(buff != NULL)
			free(buff);
...
void processPoliceRequests(int clientFD, struct userInfo userId) {
	int n;
	char *buff = NULL;
	sendMsgToClient(clientFD, "-> Enter B for Available Balance of all users \n-> C to see balance or Mini-Statement of particular Customer\n-> Enter exit to terminate");
	while(1) {
		if(buff != NULL)
			free(buff);
		buff = receiveMsgFromClient(clientFD);
		...
		else if(strcmp(buff,"C") == 0)
		{
			...
			if(checkIfUserExists(buff)) {
				...
				sendMsgToClient(clientFD, "-> Enter B to check balance\n-> M to see Mini-Statement");
					...
					buff = receiveMsgFromClient(clientFD);
					if(strcmp(buff, "M") == 0) {
						printMiniStatement(clientFD, userName);
					} else if(strcmp(buff, "B") == 0) {
			...
			{
				sendMsgToClient(clientFD, "User does not exist");
			}
			...
			sendMsgToClient(clientFD, "Unknown Query : Select from the given options");
		}
	}
	if(buff != NULL)
		free(buff);
	...
	while(1) {
		...
		if(strcmp(buff, "exit") == 0) {
			...
			*userName = (char*)malloc((n+1)*sizeof(char));
			...
			sendMsgToClient(clientFD, "User does not Exist. Please enter a valid username.");
		}
	}
	if(buff != NULL)
		free(buff);
